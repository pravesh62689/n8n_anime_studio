    ...
    models = [
        {
            ...
            "key":   os.getenv("OR_LLAMA_KEY") or os.getenv("OR_KEY_LLAMA"),
        },
        {
            "name":  "Nemotron 3 Super 120B",
            "model": "nvidia/nemotron-3-super-120b-a12b:free",
            "key":   os.getenv("OR_NEMOTRON_KEY") or os.getenv("OR_KEY_NEMOTRON_SUPER"),
        },
        {
            "name":  "Qwen 3 Coder",
            "model": "qwen/qwen3-coder:free",
            "key":   os.getenv("OR_KEY_QWEN_CODER") or os.getenv("OR_QWEN_KEY") or os.getenv("OR_KEY_QWEN3"),
        }
    ]
...
def generate_hindi_script(topic: str, duration_minutes: int = 4) -> list:
    ...
    for idx, chap in enumerate(chapters, start=1):
        ...
        prompt = (
            f"You are a professional children's cartoon director and scriptwriter. Write Chapter {idx} of a script "
            f"about '{topic}'. {chap['name']}: {chap['focus']}. "
            f"Write the narration in clear, engaging Hindi. Write the visual scene description in English. "
            f"We need exactly 5 short sequential scenes for this chapter. "
            f"Return ONLY a strictly valid JSON array — no markdown fences, no extra text. "
            f"Format: [{{\"scene_prompt\": \"detailed visual description in English\", \"narration\": \"Hindi spoken text\"}}]"
            f"\nContext from previous chapters: {json.dumps(full_script[-5:]) if full_script else 'None'}"
            f"\n{rag_context}"
        )
        ...
        for scene in chapter_scenes:
            # Seed Art Style Anchor Injection
            scene["scene_prompt"] = f"cute 2D children's cartoon style, vibrant colors, seed=42, highly consistent character design, {scene['scene_prompt']}"
...
async def generate_anime_video(prompt: str, filename: str, duration: float = 8.0, is_hero: bool = False, conditioning_image: str = None) -> str:
    """
    Generates an animated video clip.
    If is_hero is True, it attempts to use Gradio client to call the public Wan-AI/Wan2.1 Space.
    Otherwise (or if HF Space calls fail), it falls back to 2D parallax zoompan animation via rembg.
    """
    import os
    import asyncio
    import httpx
    ...
    if is_hero:
        try:
            print(f"[VFX] Attempting hero beat generation via HF Space: Wan-AI/Wan2.1...")
            ...
            if conditioning_image and os.path.exists(conditioning_image):
                ...
                print(f"[VFX] Launching i2v_generation_async conditioned on: {conditioning_image}...")
                ...
                # Text-to-Video Start
                print(f"[VFX] Launching t2v_generation_async...")
                res = await asyncio.to_thread(
                    client.predict,
                    prompt=prompt,
                    size="1280*720",
            ...
            # Poll /status_refresh to get the completed video file
            # The Wan2.1 queue is typically 5-15 min on free GPU. We must wait.
            est_wait = 600  # default
            if isinstance(res, (list, tuple)) and len(res) >= 2:
                est_wait = max(int(res[1]) if isinstance(res[1], (int, float)) else 600, 120)
            max_polls = max(est_wait // 10 + 12, 90)  # at least 15 min
            print(f"[VFX] Polling status_refresh for completion (est. wait: {est_wait}s, max polls: {max_polls})...")
            video_file = None
            for poll in range(max_polls):
                await asyncio.sleep(10)
                try:
                    status_res = await asyncio.to_thread(client.predict, api_name="/status_refresh")
                    ...
                    # When done: generated_video = {'video': '/path/to/file.mp4', 'subtitles': None}
                    # When pending: generated_video = {'value': None, '__type__': 'update'}
                    if generated_video and isinstance(generated_video, dict):
                        ...
                        if vid_path and isinstance(vid_path, str) and os.path.exists(vid_path) and os.path.getsize(vid_path) > 1000:
                            video_file = vid_path
                            print(f"[VFX] ✅ MOTION VIDEO generated! Path: {video_file} ({os.path.getsize(vid_path)} bytes)")
                            break
                    ...
                    print(f" [VFX] Poll {poll+1}/{max_polls} | ETA: {est_remaining}s | Progress: {prog_label}")
                except Exception as pe:
                    print(f" [VFX] Status poll warning: {pe}")
                    
            if video_file and os.path.exists(video_file) and os.path.getsize(video_file) > 1000:
                shutil.copy(video_file, str(output_path))
                print(f"[VFX] Copied motion video to {output_path}")
                return str(output_path)
            else:
                print("[VFX] Wan2.1 queue timed out. Falling back to parallax zoompan...")
                
        except Exception as e:
            print(f"[WARNING] Hero generation failed on Space Wan-AI/Wan2.1: {e}. Falling back to parallax zoompan...")
            
    # Tier 2: Enhanced Parallax Zoompan using rembg
    print(f"[VFX] Rendering 2D parallax zoompan for scene: {filename}...")
    ...
    # Extract foreground layer with rembg
    try:
    ...
    # Build filter complex for layered parallax effect
    filter_complex = (
...
async def synthesize_hindi_audio(text: str, base_filename: str, character: str = "narrator", scene_idx: int = 1, language: str = "hi") -> dict:
    """
...
# ---------------------------------------------------------
async def render_scene_60fps(video_path: str, audio_path: str, subtitle_path: str, base_filename: str) -> dict:
    """
    Composites voiceover, dynamic zoompan parallax video, and subtitles into a 60 FPS scene.
    Automatically ducks assets/bg_music.mp3 under the dialogue if present.
    """
    import asyncio
    ...
    bg_music = Path.cwd() / "assets" / "bg_music.mp3"
    
    if bg_music.exists():
        print(f"[Compositor] Integrating background music loop using sidechain compression...")
        filter_complex = (
...
# ---------------------------------------------------------
# 5. LOCAL BINARY RAG (ZERO RAM USAGE)
# ---------------------------------------------------------
_embedder = None
...
def get_local_embedder():
    ...
    if _embedder is None:
        ...
        print("[RAG] Loading local SentenceTransformer model (all-MiniLM-L6-v2) on CPU...")
        _embedder = SentenceTransformer("all-MiniLM-L6-v2", local_files_only=True)
    return _embedder

def setup_rag_db():
    import sqlite_vec
    db = sqlite3.connect("analytics_rag.db")
    db.enable_load_extension(True)
    sqlite_vec.load(db)
    db.enable_load_extension(False)
    db.execute('''CREATE VIRTUAL TABLE IF NOT EXISTS vec_analytics USING vec0(
                  embedding bit[384], +video_id TEXT, +performance_data TEXT)''')
    return db

def ingest_performance(video_id: str, script_text: str, retention_score: int):
    """Uses local CPU embedding and stores 1-bit binary vectors."""
    model = get_local_embedder()
    float_emb = model.encode(script_text).tolist()
    
    # Convert to 1-bit binary (48 bytes for 384 dimensions)
    binary_emb = bytearray(48)
    for i, val in enumerate(float_emb):
        if val > 0:
            binary_emb[i//8] |= (1 << (i%8))
        
    db = setup_rag_db()
    try:
        db.execute("INSERT INTO vec_analytics(embedding, video_id, performance_data) VALUES (vec_bit(?), ?, ?)", 
                   (bytes(binary_emb), video_id, json.dumps({"retention": retention_score, "script": script_text})))
        db.commit()
    finally:
        db.close()
...
def retrieve_reinforced_context(topic: str) -> str:
    """Queries RAG database for nearest neighbors using Hamming distance."""
    model = get_local_embedder()
    float_emb = model.encode(topic).tolist()
    ...
    db = setup_rag_db()
    try:
        # Query database using Hamming distance
        print("[RAG Query] Searching SQLite database using vec_distance_hamming()...")
        rows = db.execute(
    ...
    for r in rows:
        perf = json.loads(r[0])
        retention = perf.get("retention", 0)
        if retention > best_retention:
            best_retention = retention
            best_script = perf.get("script")
